% simulate_FHN_model.m is the matlab interface to the cvodes mex
%   which simulates the ordinary differential equation and respective
%   sensitivities according to user specifications.
%
% USAGE:
% ======
% [...] = simulate_FHN_model(tout,theta)
% [...] = simulate_FHN_model(tout,theta,kappa,data,options)
% [status,tout,x,y,sx,sy] = simulate_FHN_model(...)
%
% INPUTS:
% =======
...
function varargout = simulate_FHN_model(varargin)
...
if(nargin>=4)
    if(isempty(varargin{4}));
        data=amidata(length(tout),0,0,options_ami.nmaxevent,length(kappa));
    else
...
sol = ami_FHN_model(tout,theta(1:5),kappa(1:0),options_ami,plist,pbar,xscale,data);
if(options_ami.sensi==1)
end
