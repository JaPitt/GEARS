...
Int_opts.atol = 1E-6; % Absolute integration tolerance. (scaler)

Int_opts.rtol = 1E-6; % Relative integration tolerance. (scaler)
...
MEIGO_options.maxeval = 5000; % Maximum number of function evaluations. (scaler)
...
MEIGO_options.log_var = 1:5;
% 1:length(Model_information.Fitting_params); % The indices of variables that should be searched for in log space.
...
Max_number_subplots = 10; % The maximum number of subplots that should be contained in a single figure. (scaler)

%%% Post analysis options

Calculate_NRMSE = true; % If NRMSEs should be calculated. (logical)
...
Create_figure_report = true; % If GEARS should create a pdf report containing all figures. Requires Ghostscript. (logical)
    ...
    Ghostscript_path = 'C:\Program Files (x86)\gs\gs9.19\bin\gswin32c.exe'; % The path to the Ghostscript exe. Only required for the figure report. Not needed for linux. (string)   
...
Run_standard_optimisation = true; % If GEARS should only perform a singular (non-regularised) global optimisation without parameter bounding. This can be used to get insights for the convergence criterion. (logical) [Optional]
