...
global hjfun hjxl hjxu hjcl hjcu hjweight hjtolc
    %%% Jake Convert only local search to linear scale % Start
    if log_level == 2 
        
    X0(log_var) = exp(X0(log_var));
    
    x_L(log_var) = exp(x_L(log_var));

    x_U(log_var) = exp(x_U(log_var));

    end
    %%% Jake Convert only local search to linear scale % Start
        
    %%% Jake Set up to use the cost and jacobian interfaces. % Start
    if log_level == 3
     
    varargin{end+1} = log_var;    
        
    varargin{end+1} = fobj;    
        
    fobj = @Cost_log_level_3;
    
        if ~isempty(fjac)
            
        varargin{end+1} = fjac;     
            
        fjac = @Jac_log_level_3;  
        
        end
        
    end
    %%% Jake Set up to use the cost and jacobian interfaces. % End
