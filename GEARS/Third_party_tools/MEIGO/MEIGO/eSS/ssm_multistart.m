...
% maxeval      = opts.maxeval;
    ...
    %%% Jake convert all operations into log scale (if == 2 local solver will be transformed back). % Start
    if log_level > 1
    ...
    %%% Jake convert all operations into log scale (if == 2 local solver will be transformed back). % End
...
if local_solver
    ...
    % Jake fix 'auto' ndiverse bug % Start
    
    if ischar(opts.ndiverse)
    
    opts.ndiverse = 10*nvar;
    
    end
    
    % Jake fix 'auto' ndiverse bug % End
...
tic
    ...
    multix_dummy = multix(1,:);
    
    multix_dummy(log_var) = exp(multix_dummy(log_var));
    
    f0(1)    = feval(problem.f,multix_dummy,varargin{:});  
...
for ind_search = 1:nrun
    ...
    if strcmp(local_solver,'mix')
        ...
        % Jake make sure we evualate the value and not log of the value % Start 
        if log_level > 1
           
        x0_dummy = x0;
        
        x0_dummy(log_var) = exp(x0_dummy(log_var));
            
        f0(ind_search) = feval(this_f,x0_dummy,varargin{:});    
        ...
        % Jake make sure we evualate the value and not log of the value % End 
...
%   try
        if isFjacDefined
        [x,fval,exitflag,numeval]=ssm_localsolver(x0,x_L,x_U,c_L,c_U,neq,length(R),int_var,bin_var,fobj,fjac,...
        this_local_solver,opts.local.iterprint,opts.local.tol,weight,nconst,tolc,opts.local,log_level,log_var,varargin{:});
        else
        [x,fval,exitflag,numeval]=ssm_localsolver(x0,x_L,x_U,c_L,c_U,neq,length(R),int_var,bin_var,fobj,[],...
        this_local_solver,opts.local.iterprint,opts.local.tol,weight,nconst,tolc,opts.local,log_level,log_var,varargin{:});
        end
%   catch
...
% ssm_delete_files(local_solver,c_U);
    ...
    pos = zeros(size(Results_multistart.x0, 1), length(x_L));
    
    pos(:, log_var) = 1;
    
    pos = pos == 1;
            
    Results_multistart.x0(pos) = exp(Results_multistart.x0(pos));
      
    Results_multistart.xxx(pos) = exp(Results_multistart.xxx(pos));
    
    Results_multistart.xbest(log_var) = exp(Results_multistart.xbest(log_var));
        
    pos = zeros(size(Results_multistart.bestit, 1), length(x_0));
    
    pos(:, log_var) = 1;
    
    pos = pos == 1;
    
    Results_multistart.bestit(pos) = exp(Results_multistart.bestit(pos));
